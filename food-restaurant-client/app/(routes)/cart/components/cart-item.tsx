...
const CartItem = ({item}:CartItemProps) => { 
  ...
  return (
   <Box className=" flex items-center gap-4 border border-gray-200 p-3 rounded-lg">
        ...
        <div>
            <h2 className="w-full min-w-44 whitespace-nowrap truncate font-medium text-2xl text-neutral-700">
                {item.name}
            </h2> 
         ...
         <Box className="flex items-center justify-center h-full">
            <div className="flex items-center gap-2">
                {[1,2,3,4,5].map((number) => ( 
                    <div key={number} 
                         className={`w-8 h-8 cursor-pointer rounded-full flex items-center justify-center border border-hero ${qty === number 
